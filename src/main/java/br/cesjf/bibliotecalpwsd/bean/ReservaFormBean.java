...
@Named
@ViewScoped
public class ReservaFormBean extends CommandFormBean implements Serializable {
	...
	@Override
	public void clear() {
		reserva = Reserva.Builder.newInstance().build();
	}
	...
	public void calcularExemplaresPermitidos(SelectEvent event) {
		...
		if (dataReserva != null) {
			if (livro == null) {
				...
				for (Exemplar e : exemplares) {
					if (e.getIdLivro().getId().equals(livro.getId()) && e.getCircular()) {
						exemplaresPermitidos.add(e);
					}
				}
			}
			...
			for (Exemplar e : lista) {
				for (Emprestimo emp : EmprestimoDAO.getInstance().getList()) {
					if (emp.getIdExemplar().getId().equals(e.getId())) {
						if (emp.getDataDevolucao() == null && emp.getDataEmprestimo().compareTo(dataReserva) <= 0
								&& emp.getDataDevolucaoPrevista().compareTo(dataReserva) >= 0) {
							exemplaresPermitidos.remove(e);
							continue;
						}
						if (emp.getDataDevolucao() != null && emp.getDataEmprestimo().compareTo(dataReserva) <= 0
								&& emp.getDataDevolucao().compareTo(dataReserva) >= 0) {
							exemplaresPermitidos.remove(e);
							continue;
						}
					}
				...
				for (Reserva r : ReservaDAO.getInstance().getList()) {
					if (r.getIdExemplar().getId().equals(e.getId())) {
						if (!r.getCancelada() && r.getDataReserva().compareTo(dataReserva) <= 0
